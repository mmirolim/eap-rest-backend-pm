...
module.exports.sockets = {
  ...
  // mixes in socket.io events for your routes and blueprints automatically.
    ...
    /** Important register all sockets on connect and delete them on user disconnect
     *  to prevent memory leak in redis
     * @param session
     * @param socket
     * @todo Test more Mirolim Mirzakhmedov
     */
    onConnect: function(session, socket) {

        // join appropriate rooms
        if (!_.isNull(session.user) && typeof session.user !== 'undefined') {
            var user = session.user;
            // subscribe to all appropriate rooms to all groups, project and own room
            var allGroups = [];
            if (user.hasOwnProperty('groups')) {
                allGroups = _.map(user.groups, function (o) { return o.id;});
            }
            allGroups.push(user.grp);
            _(allGroups).forEach(function (o) {
                var id;
                if (typeof o === 'object') {
                    id = o.id;
                } else {
                ...
                sails.sockets.join(sails.sockets.id(socket), "groupID#" + id);
            });
            if (user.hasOwnProperty('projects')) {
                _(user.projects).forEach(function (p) {
                    sails.sockets.join(sails.sockets.id(socket), "projectID#" + p.id);
                });
            }
            sails.sockets.join(sails.sockets.id(socket), "userID#" + user.id);
  ...
  onDisconnect: function(session, socket) {
        ...
        if (!_.isNull(session.user)  && typeof session.user !== 'undefined') {
          ...
          _(allGroups).forEach(function (o) {
              ...
              sails.sockets.leave(sails.sockets.id(socket), "groupID#" + id);
          });
          if (user.hasOwnProperty('projects')) {
              _(user.projects).forEach(function (p) {
                  sails.sockets.leave(sails.sockets.id(socket), "projectID#" + p.id);
              });
          }
          sails.sockets.leave(sails.sockets.id(socket), "userID#" + user.id);
